...
// helper variables
const duration = 450, resetTreeButton = jquery_1.default('#resetTree'), expandTreeButton = jquery_1.default('#expandTree');
...
const tooltipElements = function (d) {
    const closeButton = '<div id="b2">' + closeIcon + '</div>', learnButton = '<div id="b1">Learn more</div>', name = '<div class="tip_prop">' + d.data.name + ' </div>', description = '<div class="tip_desc">' + d.data.description + '</div>', type = '<div class="tip_type">' + d.data.type + '</div>', enums = '<div class="tip_enums">' + d.data.type.replaceAll('|', ' | ') + '</div>', controlType = '<div class="tip_control">' + d.data.control + '</div>';
    ...
    if (d.data.artifact)
        elements += '<div class="tip_type">' + split(d.data.artifact) + '</div>';
    if (d.data.control)
...
function centerNode(source, ratio) {
    const r = ratio ? ratio : 2, node = d3.zoomTransform(svg.node());
    ...
    d3.select('svg#tree-container')
        .transition()
        .duration(duration)
        .call(zoomListener.transform, d3.zoomIdentity.translate(x, y).scale(node.k));
}
...
function diagonal(d) {
    ...
    if (d.source.id)
        nodeGroup = d3.select('#n-' + d.source.id).node();
...
function filterSearchItems(searchTerm) {
    jquery_1.default('#search-result li').each(function () {
        const currentText = jquery_1.default(this)
            .text()
            .toUpperCase();
...
function clickTitle(event, d) {
    ...
    // recover the mouse position to control position of tooltip
    tooltipDiv.style('left', event.pageX + 20 + 'px').style('top', event.pageY + 20 + 'px');
}
...
function update(source) {
    ...
    childCount(0, root);
    const newHeight = d3.max(levelWidth) * 40; // pixels per line
    // set the new tree layout
    treeLayout = treeLayout.size([newHeight, innerWidth]);
...
function createSearch(items) {
    const searchResult = jquery_1.default('ul#search-result'), searchInput = jquery_1.default('#search-input');
