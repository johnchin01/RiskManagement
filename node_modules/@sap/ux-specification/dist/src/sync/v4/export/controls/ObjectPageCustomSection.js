...
__decorate([
    decorators_1.syncRule({
        manifest: {
            path: (entitySet, manifest, sectionId) => application_1.getCustomSectionPath(entitySet, manifest, sectionId, 'position'),
            key: 'anchor'
        }
    }),
    ...
    decorators_1.syncRule({
        manifest: {
            ...
            import: (manifest) => {
                ...
                if (!manifest.type || manifest.type.startsWith(v4_1.CustomSectionViewTypesV4.XML)) {
                    return v4_1.CustomSectionViewTypesV4.XML;
                }
                return '';
            },
            export: (manifest, object) => {
                ...
                if ('className' in object) {
                    if (object.className === common_1.SAPUI5_FRAGMENT_CLASS) {
                        type += 'Fragment';
                    }
                    else {
            ...
            import: (manifest) => {
                ...
                if (!manifest.type || manifest.type.startsWith(v4_1.CustomSectionViewTypesV4.XML)) {
                    if (!manifest.type || manifest.type === 'XMLFragment') {
                        return common_1.SAPUI5_FRAGMENT_CLASS;
                    }
                    else {
                        return manifest.type.slice(v4_1.CustomSectionViewTypesV4.XML.length);
                    }
                }
