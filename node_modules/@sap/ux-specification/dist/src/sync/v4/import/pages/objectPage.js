...
function fillTableColumnsInConfig(importParameters, manifestSection, configPart, sectionId, factory, v4Page) {
    if (manifestSection['columns']) {
        ...
        const columnsInSchema = importParameters.jsonSchema['definitions'][sectionId].properties;
        if (!columnsInSchema) {
            return;
        }
        const manifestSectionId = sectionId.replace(/::/g, '/');
        for (const columnKey in columnsInSchema) {
            const column = manifestSection['columns'][columnKey];
            if (column && !column.template) {
                const columnInSchema = columnsInSchema[columnKey];
                const schemaPropertyName = columnInSchema.$ref.split(common_2.DEFINITION_LINK_PREFIX)[1];
                configPart[sectionId].table.columns[columnKey] = factory.createInstance(common_1.PageTypeV4.ObjectPage, schemaPropertyName);
                utils_1.transferSettingsOfObject(configPart[sectionId].table.columns[columnKey], importParameters.manifest, importParameters.jsonSchema['definitions'][schemaPropertyName], v4Page.id, [columnKey], manifestSectionId);
            }
        }
    }
}
