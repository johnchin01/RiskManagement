...
function handleActionRecord(appSchema, entityType, actions, lineItemPath, lineItemRecord, lineItemId, recordIndex) {
    ...
    if (lineItemRecord.Determining !== true) {
        ...
        const actionType = lineItemId === 'LineItems' ? '' : 'ObjectPage';
        const actionId = common_1.prepareRef(lineItemRecord.Action);
        const actionDefinition = `${actionType}ToolBarAction<${actionId}>`;
        ...
        const actionReferenceType = actionType === '' ? 'Action' : `${actionType}ToolBarAction`;
        if (appSchema['definitions'][actionReferenceType]) {
            appSchema['definitions'][actionDefinition] = Object.assign(Object.assign({}, JSON.parse(JSON.stringify(appSchema['definitions'][actionReferenceType]))), { description, annotationPath: `${lineItemPath}/${recordIndex}` });
        }
        else {
